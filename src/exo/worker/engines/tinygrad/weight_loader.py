...
def _normalize_tensor_dtype_for_device(tensor: Tensor) -> Tensor:
    """Normalize loaded model tensors before transferring them to the active tinygrad device.

    Adapted from Scottcjn/exo-cuda's OpenCL/AMD compatibility work: USE_FP32=1
    keeps older OpenCL/Vulkan-adjacent devices alive by avoiding fp16/bf16 kernels.
    """
    if getenv("USE_FP32", 0) and tensor.dtype in (dtypes.bfloat16, dtypes.float16):
    ...
    if Device.DEFAULT == "CLANG":
        return tensor.llvm_bf16_cast(dtypes.float32).to(tensor.device)

    if getenv("SUPPORT_BF16", 1):
        return tensor.cast(dtypes.float16)

    return tensor.llvm_bf16_cast(dtypes.half).to(tensor.device)
    ...
    with Context(BEAM=0):
        for safetensor_file in sorted(path.glob("*.safetensors")):
            ...
            for key, tensor in shard.items():
                merged[key] = _normalize_tensor_dtype_for_device(tensor).to(Device.DEFAULT).contiguous().realize()  # pyright: ignore[reportUnknownMemberType]
